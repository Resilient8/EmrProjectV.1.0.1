...
exports.registerCypressOverwrites = void 0;
function isCheckBasedComponent(subject) {
    return (subject.hasClass('q-checkbox') ||
        subject.hasClass('q-toggle') ||
        subject.hasClass('q-radio'));
}
function registerCypressOverwrites() {
    Cypress.Commands.overwrite('select', (originalFn, subject, valueOrTextOrIndex, options) => {
        ...
        if (subject.hasClass('q-select')) {
            if (Array.isArray(valueOrTextOrIndex)) {
                if (!subject.hasClass('q-select--multiple')) {
                    throw new Error('Cypress: select command with array param can only be used with a multiple select');
                }
            }
            else {
            ...
            if (valueOrTextOrIndex.length === 0) {
                throw new Error('Cypress: select command requires at least one value');
            }
            cy.wrap(subject).click();
            cy.withinSelectMenu(() => {
                valueOrTextOrIndex.forEach((value) => {
    ...
    // @ts-expect-error
    chai.Assertion.overwriteProperty('checked', (_super) => {
        return function () {
            const subject = this._obj;
            if (subject && isCheckBasedComponent(subject)) {
                const expectedValue = this.__flags.negate ? 'false' : 'true';
                new chai.Assertion(subject[0]).to.have.attr('aria-checked', expectedValue);
            }
            else {
                _super.call(this);
            }
...
exports.registerCypressOverwrites = registerCypressOverwrites;
//# sourceMappingURL=cypress-overrides.js.map